// Generates specimen/index.html: the browsable specimen sheet.
// Consumes .cache/rows.json, produced by build.mjs.
...
// dark enough on light paper? otherwise darken it; too dark on a dark ground? lighten it
...
const cells=rows.map(r=>`
      <article class="cell" data-fam="${esc(r.family)}" data-slug="${r.slug}">
        <div class="glyph">
        ...
        <button class="copy" type="button" data-slug="${r.slug}">Copy the SVG</button>
      </article>`).join('');
...
const tableRows=rows.map(r=>`<tr><th scope="row">${esc(r.label)}</th><td><code>${esc(r.tabler)}</code></td><td><code>${esc(r.lucide)}</code></td><td>${r.simpleIcons?`<code>${esc(r.simpleIcons)}</code>`:'<span class="none">no official logo</span>'}</td></tr>`).join('\n        ');

const svgData=JSON.stringify(Object.fromEntries(rows.map(r=>[r.slug,{tabler:r.tSvg,lucide:r.lSvg}])));

const html=`<title>Protocol iconography</title>
...
<header class="top">
  <div class="wrap">
    <p class="eyebrow">Specimen · 32 protocols · free SVGs</p>
    <h1>Iconography of network protocols</h1>
    <p class="lede">SSH, HTTP, FTP and REST <strong>have no official logo</strong> — they are specifications, not brands. Consistency therefore cannot come from logos: it comes from choosing <strong>a single icon family</strong> for every protocol. Here is that set, built in two free families with an identical stroke, plus the few protocols that really do own a mark.</p>
    <div class="facts">
      <div class="fact"><b>32</b><span>protocols covered</span></div>
      <div class="fact"><b>13</b><span>have a real brand logo</span></div>
      <div class="fact"><b>24 px</b><span>shared grid, 2 px stroke</span></div>
      <div class="fact"><b>MIT · ISC · CC0</b><span>commercial use allowed</span></div>
    </div>
  </div>
</header>
...
<div class="toolbar">
  <div class="wrap">
    <div class="seg" role="group" aria-label="Icon family">
...
<div class="wrap">
  <section>
    <div class="shead"><h2>The consistent set</h2><p>Same grid, same stroke, same corners — switch between the two families to compare.</p></div>
    <div class="grid" id="grid">${cells}
    </div>
  </section>

  <section>
    <div class="shead"><h2>The protocols that do have a logo</h2><p>Official marks redistributed by Simple Icons, in their original colour.</p></div>
    <div class="bgrid">${brand}
    </div>
  </section>

  <section>
    <div class="shead"><h2>Sources and licences</h2><p>Three repositories, all usable in a commercial project.</p></div>
    <div class="notes">
      <div class="note">
        <h3>Tabler Icons</h3>
        <p>5,900+ line icons, 24 px grid, 2 px stroke. The richest one for network and server vocabulary.</p>
        <span class="lic">MIT · tabler.io/icons</span>
      </div>
      <div class="note">
        <h3>Lucide</h3>
        <p>A community fork of Feather. A finer, rounder stroke, ideal for a light interface.</p>
        <span class="lic">ISC · lucide.dev</span>
      </div>
      <div class="note">
        <h3>Simple Icons</h3>
        <p>3,400+ monochrome brand logos with their official colour. This is where MQTT, GraphQL, XMPP and WireGuard live.</p>
        <span class="lic">CC0 1.0 · simpleicons.org</span>
      </div>
      <div class="note">
        <h3>Equivalent alternatives</h3>
        <p>Phosphor (MIT), Material Symbols (Apache 2.0), Bootstrap Icons (MIT), Iconoir (MIT), Font Awesome Free (CC BY 4.0, attribution required). Iconify aggregates them all behind one API.</p>
        <span class="lic">Same method, another stroke</span>
      </div>
    </div>
  </section>

  <section>
    <div class="shead"><h2>Correspondence table</h2><p>The exact names to copy into your code.</p></div>
    <div class="scroller">
      <table>
        <thead><tr><th scope="col">Protocol</th><th scope="col">Tabler (MIT)</th><th scope="col">Lucide (ISC)</th><th scope="col">Official logo (CC0)</th></tr></thead>
        <tbody>
        ${tableRows}
        </tbody>
      </table>
  ...
  <div class="warn">
    <h2>Two precautions before publishing</h2>
    <p><strong>Free of rights ≠ free of trademark.</strong> The MIT, ISC and CC0 licences cover the SVG file, not the mark it depicts. The GraphQL, MQTT, WireGuard and RabbitMQ logos remain the property of their holders: they are used to designate the technology, never to suggest a partnership or a certification. Every brand publishes its own usage rules.</p>
    <p><strong>Do not mix families.</strong> A Tabler <code>world-www</code> next to a Lucide <code>globe</code> shows immediately: the weights and the corners differ. Pick one family, and introduce the coloured brand logos only in an area clearly separated from the rest of the interface.</p>
  </div>
</div>
...
(function(){
  ...
  grid.addEventListener('click',function(e){
    ...
    if(navigator.clipboard&&navigator.clipboard.writeText){
      navigator.clipboard.writeText(svg).then(function(){say('SVG copied — '+body.dataset.set);},function(){say('Copy refused by the browser');});
...
console.log(`  specimen/index.html · ${(html.length / 1024).toFixed(0)} kB`);
